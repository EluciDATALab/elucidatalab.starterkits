...
def get_data(data_path=None, force=False):
    ...
    def extract_csv_file(filename):
        target_folder = data_path / 'SK_4_1'
        zip_fname = pipeline.download_from_repo(
            'SK_4_1',
            'IntelligentDataRetention.zip',
            str(target_folder / 'IntelligentDataRetention.zip'),
            force=force)

        with zipfile.ZipFile(zip_fname, 'r') as zip:
            zip.extract('IntelligentDataRetention.csv', path=str(target_folder))
        os.remove(zip_fname)
        return filename
...
def load_data(data_path=None, force=False):
    ...
    df = get_data(data_path=data_path, force=force)
...
def new_window(CD, c, d):
    ...
    upper = (c[0], c[1] + CD)
    lower = (c[0], c[1] - CD)
    upper_max = define_slope(upper, d)
    lower_min = define_slope(lower, d)
