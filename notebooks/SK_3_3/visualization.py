...
def plot_train_delays(df, type='day'):
    """
    Plot train delay distribution per day of the week or per month

    :param df. a pandas dataframe that contains OTP data
    :param type. how to visualize the delays. available values are day and moth. Default: day
    """
...
def plot_train_density(df, percentile=75):
    ...
    bars = ax.bar(train_density.theta, train_density.ct, width=width, bottom=2)
...
def plot_delays_per_stop_number(df):
    """
    Plot delay as a function of the stop number
    ...
    sb.boxplot(x='rk_stop', y='delay', data=df.query("rk_stop<=20"), ax=axes[0])
    axes[0].set_title('Boxplot distribution of delays per stop number')
    ...
    # calculate median delay per stop number
    delay_stop = df.groupby('rk_stop').delay.mean().reset_index(name='avg')
    sb.regplot(x='rk_stop', y='avg', data=delay_stop, ax=axes[1])
    axes[1].set_title('Mean delay vs stop number')
    axes[1].set_ylabel('Mean delay (min)')
    axes[1].set_xlabel('Stop number')
    a = axes[1].set_ylim([0, delay_stop.avg.max()]);

def plot_train_run(df_train_view, uni_coords, uni_runs, df_distance, df_coords, train_id='216', date='2016-10-07'):
    """
    Visualize train run

    :param df. a pandas dataframe with train view data
    ...
    # rand_run = df_otp.merge(uni_runs.sample(1),on = ['train_id','date'])
    ...
    edge_coords = df_distance[df_distance.next_station.isin(dest_count.dest.unique())]\
                            [['next_station','lat','lon']].drop_duplicates()
    ...
    plt.scatter(rand_run.lon, rand_run.lat,marker='o', c=rand_run.rk_stop, s=200)
...
def plot_distance_distribution(df):
    ...
    g = sb.distplot(df.distance)

def delay_reg_plot(df, uni_runs):
    """
    regression plot to visualize delays as a function of distance between stations and cumulative distance of train run
    ...
    df_sub = df.merge(uni_runs.sample(1000), on=['train_id','date'])

    f, axes = plt.subplots(1, 2)
    # plot delays per stop
    sb.regplot(x='distance', y='delay', data=df_sub.query("distance < 10"), ax=axes[0])
    axes[0].set_title('Delay as a function of distance between stations (limited to distances < 10km)')
    axes[0].set_ylabel('Delay (min)')
    axes[0].set_xlabel('Distance (km)')

    # calculate median delay per stop # and display
    sb.regplot(x='cum_distance', y='delay', data=df_sub, ax=axes[1])
    axes[1].set_title('Delay as a function of cumulative distance of train run')
...
def total_train_run_distance_reg_plot(df):
    """
    ...
    f, axes = plt.subplots(1, 2)
    # plot delays per stop
    sb.regplot(x='distance', y='delay_sum', data=df_run, ax=axes[0])
    axes[0].set_title('Total train run delay as a function total run distance')
    axes[0].set_ylabel('Delay (min)')
    axes[0].set_xlabel('Distance (km)')

    # calculate median delay per stop # and display
    sb.regplot(x='distance', y='delay_last', data=df_run, ax=axes[1])
    axes[1].set_title('Delay at last stop as a function total run distance')
    axes[1].set_ylabel('Delay (min)')
...
def plot_northbound_southbound_dely_per_station(df):
    ...
    a = axes.add_line(plt.Line2D(ax_lims, ax_lims, color='black'))
...
def plot_average_delay(df, percentile=90):
    ...
    :param df. a pandas dataframe with OTP data
    :param df. percentile value to use for setting the threshold. Default: 90
    """
...
def plot_track_change_frequency_with_delays(df_track_change_frequency, df_otp):
    ...
    g = sb.lmplot(x='track_change_frequency', y='delay', data=track_change_delay)
    g.set_xlabels('Track change frequency')
    g.set_ylabels('Mean delay (min)')

    from scipy.stats import pearsonr
    pearson_r = pearsonr(track_change_delay.delay, track_change_delay.track_change_frequency)
    print("Pearson's coefficient = %0.2f with p-value = %0.2e" % (pearson_r[0], pearson_r[1]))
...
def plot_lowest_track_change_frequency(track_change_frequency, n_stations=10):
    ...
    :param n_stations. Number of stations to plot
    """
    g = sb.barplot(x='next_station', y='track_change_frequency', data=track_change_frequency.tail(n_stations).iloc[::-1])
    g.set_xlabel('')
    g.set_ylabel('Track change frequency')
    for tick in g.get_xticklabels():
        tick.set_rotation(45)